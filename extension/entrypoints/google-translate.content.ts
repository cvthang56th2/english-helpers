...
const BUTTON_HOST_ID = "word-ledger-gt-btn-host";
const UI_HOST_ID = "word-ledger-gt-ui-host";
...
function syncSaveButton(ui: ReturnType<typeof mountUi>) {
  const pair = readGoogleTranslatePair();
...
function mountUi() {
  ...
  function openConfirm(pair: GoogleTranslatePair) {
    hideCard();
    card.hidden = false;
    ...
    function render() {
      ...
      card.innerHTML = `
        ...
        <h2>Lưu từ Google Dịch</h2>
        <label>Từ
          <input id="term" value="${escapeAttr(term)}" />
        </label>
        <label>Nghĩa
          <input id="translation" value="${escapeAttr(translation)}" />
        </label>
        <label>IPA
...
const uiCss = `
  ...
  h2 { margin: 0 18px 8px 0; font-size: 16px; }
  label {
    display: grid;
    gap: 4px;
    margin-top: 8px;
  ...
  .muted { margin: 6px 0 0; font-size: 12px; color: #64748b; }
  .row .muted { margin: 0; }
